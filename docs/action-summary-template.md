...
- Start with `📝 **{Specific Meeting Title}** · {MM-DD-YYYY}`.
...
- Format topic headings as `{emoji} **{heading}**`, without Markdown heading markers.
- Keep related facts together instead of repeating them across sections.
- Add `Next Steps` only when the Session contains genuine follow-up.
- Name an owner only when the responsibility is supported by the Session.
- For reflective Dialogues, use headings for themes, findings, tensions, or open questions rather than manufacturing operational tasks.
